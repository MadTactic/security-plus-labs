...
- Enable Remote Desktop on Windows Server 2019 and add a domain user to the Remote Desktop Users group
- Establish an RDP session between two Windows systems and confirm the remote hostname
- Verify that the OpenSSH server is installed, running, and listening on a Linux host, and identify its authentication method
...
False. SSH is a protocol with long-standing implementations on BSDs, macOS, and modern Windows, plus network equipment, embedded systems, and mainframes. The protocol itself is OS-agnostic.

**2. On a standard Windows system, Remote Desktop is by default enabled for all users.**
False. RDP is disabled by default on fresh Windows installs, and once enabled only members of the Administrators or Remote Desktop Users groups can connect. That is why Rene had to be added explicitly in Part 1.

**3. What functions can the tool PuTTY be used for?**
Serial console, network file transfer, and terminal emulator. PuTTY handles SSH, Telnet, Rlogin, and raw socket connections, supports serial console access over COM ports, and ships with `pscp`/`psftp` for file transfer over SSH. It is not a web browser.

**4. When initiating an SSH connection, what is the typical CLI syntax?**
`ssh username@host`, where `host` is an IP address or resolvable hostname. The other options shown in the lab (`ssh open 22`, `ssh:///username`, `ssh id=username`) are not valid SSH syntax.

**5. What is the form of authentication supported by SSH by default?**
Password. When the `PasswordAuthentication` directive is at its default or commented out (as Part 2 showed), the OpenSSH daemon accepts password authentication. Key-based authentication is stronger but requires explicit setup with a keypair and `authorized_keys` deployment.
